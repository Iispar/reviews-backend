...
public interface ReviewRepository extends PagingAndSortingRepository<Review, Integer>, JpaRepository<Review, Integer> {
	 ...
	 */
	@Query(value = "SELECT COUNT(review_rating) AS count, ROUND(AVG(review_rating), 1) AS rating, MONTHNAME(review_date) AS time, min(YEAR(review_date)) AS timeYear FROM reviews r WHERE r.review_item IN"
	 ...
	 */
	@Query(value = "SELECT COUNT(review_rating) AS count, ROUND(AVG(review_rating), 1) AS rating, WEEK(review_date) AS time, min(YEAR(review_date)) AS timeYear FROM reviews r WHERE r.review_item IN"
			+ "(SELECT item_id FROM items i WHERE i.item_account = ?1)"
	 ...
	 */
	@Query(value = "SELECT COUNT(review_rating) AS count, ROUND(AVG(review_rating), 1) AS rating, MONTHNAME(review_date) AS time, min(YEAR(review_date)) AS timeYear FROM reviews r WHERE r.review_item = ?1 GROUP BY MONTHNAME(review_date)", nativeQuery = true)
	 ...
	 * @return list of count of reviews and their avg rating grouped by week.
	 */
	@Query(value = "SELECT COUNT(review_rating) AS count, ROUND(AVG(review_rating), 1) AS rating, WEEK(review_date) AS time, min(YEAR(review_date)) AS timeYear FROM reviews r WHERE r.review_item = ?1 GROUP BY WEEK(review_date)", nativeQuery = true)
